...
Java.perform(function () {
  ...
  function logPatch(message) {
    if (patchLogCount < 5) {
      ...
      patchLogCount++;
      if (patchLogCount === 5) {
        log("further PackageInfo patch logs suppressed");
      }
    }
  }
  ...
  try {
    ...
    getPackageInfoVersioned.implementation = function (versionedPackage, flags) {
      ...
      try {
        ...
        log("VersionedPackage package lookup failed: " + e);
      }
      return info;
    };

    log("hooked ApplicationPackageManager.getPackageInfo(VersionedPackage,int)");
  } catch (e) {
    log("getPackageInfo(VersionedPackage,int) hook skipped: " + e);
  }

  log("safe version spoof installed " + FROM + " -> " + TO);
});
